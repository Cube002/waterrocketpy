...
# Import the optimization module (assuming it's saved as water_rocket_optimizer.py)
...
def test_quick_altitude_optimization():
    ...
    result = optimize_for_altitude(
        ...
        maxiter=20,      # Small number for quick test
...
def compare_targets():
    ...
    results['altitude'] = optimize_for_altitude(bounds=common_bounds, **common_settings)
    
    print("Optimizing for velocity...")
    results['velocity'] = optimize_for_velocity(bounds=common_bounds, **common_settings)
    
    print("Optimizing for flight time...")
    results['flight_time'] = optimize_for_flight_time(bounds=common_bounds, **common_settings)
    ...
    print(f"{'Target':<12} {'Best Value':<12} {'L_body(cm)':<10} {'d_body(cm)':<10} "
          f"{'P(bar)':<8} {'Nozzle(mm)':<10} {'Water%':<8}")
    print("-" * 80)
    
    for target, result in results.items():
        params = result['best_params']
        print(f"{target:<12} {result['best_value']:<12.4f} "
...
def main():
    ...
    try:
        ...
        print(f"{'='*60}")
        print("All tests passed! The optimization module is working correctly.")
        print("\nTo use the optimizer in your own code, you can:")
        print("1. Use the convenience functions: optimize_for_altitude(), etc.")
