...
def main():
    ...
    try:
        ...
        print(f"   Initial total energy: {energy_components.total_initial_energy:.2f} J")
        print(f"   Maximum kinetic energy: {energy_components.max_kinetic_energy:.2f} J")
        print(f"   Maximum potential energy: {energy_components.max_potential_energy:.2f} J")
        print(f"   Total drag loss: {energy_components.total_drag_loss:.2f} J")
        print(f"   Total expelled energy: {energy_components.total_expelled_energy:.2f} J") #somehow not printing
        print(f"   Final energy conservation error: {energy_components.energy_conservation_error[-1]:.2f}%") #somehow not printing
        ...
        print("   Check the generated plot files for detailed energy analysis.")
...
def create_energy_plots(energy_components, flight_data):
    ...
    expelled_kinetic = (energy_components.water_out_kinetic_energy + 
                       energy_components.air_out_kinetic_energy)
    expelled_potential = (energy_components.water_out_potential_energy + 
    ...
    ax1.fill_between(time, 0, internal_energy, 
                    color=colors['internal'], alpha=0.8, label='Internal Energy (Air)')
    ax1.fill_between(time, internal_energy, internal_energy + kinetic_energy,
                    color=colors['kinetic'], alpha=0.8, label='Kinetic Energy (Rocket)')
    ax1.fill_between(time, internal_energy + kinetic_energy, 
                    internal_energy + kinetic_energy + potential_energy,
                    color=colors['potential'], alpha=0.8, label='Potential Energy (Rocket)')
    ax1.fill_between(time, internal_energy + kinetic_energy + potential_energy,
                    internal_energy + kinetic_energy + potential_energy + expelled_total,
                    color=colors['expelled'], alpha=0.8, label='Expelled Energy (Water + Air)')
    ax1.fill_between(time, internal_energy + kinetic_energy + potential_energy + expelled_total,
                    internal_energy + kinetic_energy + potential_energy + expelled_total + drag_losses,
                    color=colors['losses'], alpha=0.8, label='Energy Losses (Drag)')
    
    # Add total energy line
    total_energy = (internal_energy + kinetic_energy + potential_energy + 
                   expelled_total + drag_losses)
    ax1.plot(time, total_energy, color=colors['total'], linewidth=2, 
    ...
    if flight_data.water_depletion_time > 0:
        ax1.axvline(flight_data.water_depletion_time, color='red', linestyle='--', 
                   alpha=0.7, label='Water Depletion')
    if flight_data.air_depletion_time > 0:
        ax1.axvline(flight_data.air_depletion_time, color='orange', linestyle='--', 
                   alpha=0.7, label='Air Depletion')
    
    ax1.set_xlabel('Time (s)')
    ax1.set_ylabel('Energy (J)')
    ax1.set_title('Complete Energy Breakdown Over Time', fontsize=14, fontweight='bold')
    ax1.legend(loc='upper right', bbox_to_anchor=(1.02, 1))
    ax1.grid(True, alpha=0.3)
    
    # Detailed kinetic energy breakdown
    ax2 = fig.add_subplot(gs[1, 0])
    ax2.plot(time, energy_components.rocket_kinetic_energy, 
            color=colors['kinetic'], linewidth=2, label='Rocket Kinetic')
    ax2.plot(time, energy_components.water_out_kinetic_energy, 
            color='#74b9ff', linewidth=2, label='Expelled Water Kinetic')
    ax2.plot(time, energy_components.air_out_kinetic_energy, 
            color='#a29bfe', linewidth=2, label='Expelled Air Kinetic')
    ...
    ax3.plot(time, energy_components.rocket_potential_energy, 
            color=colors['potential'], linewidth=2, label='Rocket Potential')
    ax3.plot(time, energy_components.water_out_potential_energy, 
            color='#00b894', linewidth=2, label='Expelled Water Potential')
    ax3.plot(time, energy_components.air_out_potential_energy, 
            color='#00cec9', linewidth=2, label='Expelled Air Potential')
    ...
    ax4.plot(time, energy_components.energy_conservation_error, 
    ...
    ax5.plot(time, expelled_rate, color=colors['expelled'], linewidth=2, 
            label='Expelled Energy Rate')
    ax5.plot(time, drag_rate, color=colors['losses'], linewidth=2, 
...
def create_energy_summary_chart(energy_components):
    ...
    final_expelled = (energy_components.water_out_kinetic_energy[-1] + 
    ...
    # Create pie chart
    wedges, texts, autotexts = ax1.pie(energies, labels=labels, colors=colors, 
                                      autopct='%1.1f%%', startangle=90)
    ax1.set_title(f'Final Energy Distribution\n(Total Initial: {energy_components.total_initial_energy:.0f} J)')
    
    # Energy efficiency analysis
    efficiency_data = {
        'Useful Energy\n(Kinetic + Potential)': final_kinetic + final_potential,
        'Expelled Energy\n(Carried Away)': final_expelled,
        'Energy Losses\n(Drag)': final_losses,
        'Remaining Internal\n(Unused)': final_internal
    }
    
    ax2.bar(range(len(efficiency_data)), list(efficiency_data.values()), 
    ...
    for i, (key, value) in enumerate(efficiency_data.items()):
        ...
        ax2.text(i, value + 0.5, f'{percentage:.1f}%', 
