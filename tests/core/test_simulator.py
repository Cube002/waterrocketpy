...
class TestWaterRocketSimulator:
    ...
    def test_water_depletion_event(self):
        ...
        state_with_water = np.array(
            [10, 5, 0.5, 0]
        )  # altitude, velocity, water_mass, liquid_gas_mass
    ...
    def test_rocket_ode_with_water_and_liquid_gas(self):
        ...
        simulator.physics_engine.calculate_air_volume = Mock(
            return_value=0.001
        )
        simulator.physics_engine.calculate_water_thrust = Mock(
            return_value=(50.0, 20.0, 0.1)
        )
        simulator.physics_engine.calculate_drag = Mock(return_value=5.0)
        simulator.physics_engine.calculate_net_force = Mock(
            return_value=(45.0, 10.0)
        )
    ...
    def test_rocket_ode_with_water_only(self):
        ...
        simulator.physics_engine.calculate_pressure_adiabatic = Mock(
            return_value=5e5
        )
        simulator.physics_engine.calculate_water_thrust = Mock(
            return_value=(30.0, 15.0, 0.08)
        )
        simulator.physics_engine.calculate_drag = Mock(return_value=3.0)
        simulator.physics_engine.calculate_net_force = Mock(
            return_value=(27.0, 8.0)
        )
    ...
    def test_calculate_derived_quantities(self):
        ...
        simulator.physics_engine.calculate_temperature_adiabatic = Mock(
            return_value=280
        )
        simulator.physics_engine.calculate_water_thrust = Mock(
            return_value=(30.0, 15.0, 0.08)
        ...
        pressure, temperature, thrust, drag = (
            simulator._calculate_derived_quantities(time, states, params)
        )
    ...
    def test_simulate_basic_flight(self, mock_solve_ivp):
        ...
        simulator._calculate_derived_quantities = Mock(
            return_value=(
        ...
        assert flight_data.flight_time == 3.0
        assert (
            flight_data.water_depletion_time == 0.0
        )  # No water depletion event
    ...
    def test_simulate_with_water_depletion(self, mock_solve_ivp):
        ...
        mock_solution_thrust.t_events = [
            np.array([2.0])
        ]  # Water depletion at t=2
        ...
        mock_solve_ivp.side_effect = [
            mock_solution_thrust,
            mock_solution_coasting,
        ]
        ...
        assert isinstance(flight_data, FlightData)
        assert (
            len(flight_data.time) == 5
        )  # Combined thrust and coasting phases
        assert flight_data.water_depletion_time == 2.0
    ...
    def test_simulate_with_custom_sim_params(self, mock_solve_ivp):
        ...
        sim_params = {"max_time": 5.0, "time_step": 0.01, "solver": "RK45"}
    ...
    def test_simulate_with_validation_warnings(self, mock_print):
        ...
        simulator.validator.validate_rocket_parameters = Mock(
            return_value=[
        ...
        # Mock solve_ivp
        with patch(
            "waterrocketpy.core.simulation.solve_ivp"
        ) as mock_solve_ivp:
            mock_solution = Mock()
    ...
    def test_flight_data_acceleration_calculation(self):
        ...
        with patch(
            ...
            np.testing.assert_array_almost_equal(
                flight_data.acceleration, expected_acceleration
            )
...
class TestSimulationIntegration:
    ...
    def test_simulation_with_realistic_parameters(self):
        ...
        with patch.object(
            simulator.physics_engine, "calculate_air_volume"
        ) as mock_air_vol, patch.object(
            simulator.physics_engine, "calculate_pressure_adiabatic"
        ) as mock_pressure, patch.object(
            simulator.physics_engine, "calculate_thrust"
        ) as mock_thrust, patch.object(
            simulator.physics_engine, "calculate_drag"
        ) as mock_drag, patch.object(
            simulator.physics_engine, "calculate_net_force"
        ) as mock_net_force, patch.object(
            simulator.physics_engine, "calculate_temperature_adiabatic"
        ) as mock_temp:
