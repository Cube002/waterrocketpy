...
class ParameterExplorer:
    ...
    def _update_water_fraction(self, params: Dict, value: float):
        ...
        params['water_fraction'] = np.clip(value, 0.01, 0.99)  # Keep reasonable bounds
    ...
    def extract_base_parameters(self, rocket) -> Dict[str, float]:
        ...
        for param_name, sim_param_name in param_mapping.items():
            if sim_param_name in sim_params:
                ...
                print(f"  Using default for {param_name} = {base_params[param_name]}")
    ...
    def create_parameter_configs(self, base_params: Dict[str, float], 
        ...
        param_defaults = {
            ...
            'water_fraction': {'unit': '-', 'min_factor': 0.3, 'max_factor': 3.0},
            'nozzle_diameter': {'unit': 'm', 'min_factor': 0.5, 'max_factor': 2.5},
            'bottle_volume': {'unit': 'm³', 'min_factor': 0.5, 'max_factor': 2.0},
            'bottle_diameter': {'unit': 'm', 'min_factor': 0.7, 'max_factor': 1.5},
            'empty_mass': {'unit': 'kg', 'min_factor': 0.5, 'max_factor': 2.0},
            'drag_coefficient': {'unit': '-', 'min_factor': 0.3, 'max_factor': 3.0},
        }
        ...
        for param_name in parameter_names:
            ...
            defaults = param_defaults.get(param_name, {'unit': '', 'min_factor': 0.5, 'max_factor': 2.0})
        ...
        try:
            ...
            for param_name, value in param_values.items():
                if param_name in self.parameter_updaters:
                    ...
                    old_value = sim_params.get(self._get_sim_param_name(param_name), "NOT FOUND")
                    print(f"Debug - Old value: {old_value}")
                    
                    self.parameter_updaters[param_name](sim_params, value)
                    
                    new_value = sim_params.get(self._get_sim_param_name(param_name), "NOT FOUND")
                    print(f"Debug - New value: {new_value}")
                else:
                    print(f"Warning: No updater found for parameter {param_name}")
...
def main():
    ...
    if test_flight:
        print(f"   Test simulation successful - apogee: {test_flight.max_altitude:.2f} m")
    else:
    ...
    param_configs = explorer.create_parameter_configs(
        ...
        custom_ranges={
            'pressure': {'min_factor': 0.4, 'max_factor': 2.5, 'num_points': 5},
            'water_fraction': {'min_factor': 0.5, 'max_factor': 2.0, 'num_points': 5},
            'nozzle_diameter': {'min_factor': 0.6, 'max_factor': 2.0, 'num_points': 5}
        }
    )
    ...
    for param_name, param_config in param_configs.items():
        ...
        print(f"   Range: {param_config.min_value:.6f} to {param_config.max_value:.6f}")
        
        # Test minimum value
        min_flight = explorer.simulate_single_point(base_rocket, {param_name: param_config.min_value})
        if min_flight:
            print(f"   Min {param_name} ({param_config.min_value:.6f}): apogee = {min_flight.max_altitude:.2f} m")
        
        # Test maximum value  
        max_flight = explorer.simulate_single_point(base_rocket, {param_name: param_config.max_value})
        if max_flight:
            print(f"   Max {param_name} ({param_config.max_value:.6f}): apogee = {max_flight.max_altitude:.2f} m")
