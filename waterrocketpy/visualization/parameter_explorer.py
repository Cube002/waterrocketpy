...
class ParameterExplorer:
    ...
    def __init__(self):
        ...
        self.target_extractors = {
            ...
            'water_depletion_time': ('water_depletion_time', 's', 'Water Depletion Time'),
            'air_depletion_time': ('air_depletion_time', 's', 'Air Depletion Time'),
        }
        
        # Define parameter mappings - maps explorer parameter names to simulation parameter keys
        # Format: 'explorer_name': ('sim_param_key', 'display_name', 'unit', default_range)
        self.parameter_mappings = {
            # Pressure and initial conditions
            'initial_pressure': ('P0', 'Initial Pressure', 'Pa', {'min_factor': 0.3, 'max_factor': 3.0}),
            'pressure': ('P0', 'Initial Pressure', 'Pa', {'min_factor': 0.3, 'max_factor': 3.0}),  # alias
            
            # Water and mass parameters
            'water_fraction': ('water_fraction', 'Water Fraction', '-', {'min_factor': 0.3, 'max_factor': 2.5}),
            
            # Nozzle parameters
            'nozzle_area': ('A_nozzle', 'Nozzle Area', 'm²', {'min_factor': 0.4, 'max_factor': 3.0}),
            'nozzle_diameter': ('A_nozzle', 'Nozzle Diameter', 'm', {'min_factor': 0.5, 'max_factor': 2.5, 'convert_func': self._diameter_to_area}),
            'nozzle_discharge_coefficient': ('C_d', 'Nozzle Discharge Coefficient', '-', {'min_factor': 0.5, 'max_factor': 1.5}),
            
            # Bottle parameters
            'bottle_volume': ('V_bottle', 'Bottle Volume', 'm³', {'min_factor': 0.5, 'max_factor': 2.0}),
            
            # Mass parameters
            'empty_mass': ('m_empty', 'Empty Mass', 'kg', {'min_factor': 0.5, 'max_factor': 2.0}),
            
            # Aerodynamic parameters
            'drag_coefficient': ('C_drag', 'Drag Coefficient', '-', {'min_factor': 0.3, 'max_factor': 3.0}),
            'reference_area': ('A_rocket', 'Reference Area', 'm²', {'min_factor': 0.5, 'max_factor': 2.0}),
            'rocket_diameter': ('A_rocket', 'Rocket Diameter', 'm', {'min_factor': 0.7, 'max_factor': 1.5, 'convert_func': self._diameter_to_area}),
            
            # Liquid gas
            'liquid_gas_mass': ('liquid_gas_mass', 'Liquid Gas Mass', 'kg', {'min_factor': 0.0, 'max_factor': 10.0}),
        }
    ...
    def _update_parameter(self, params: Dict[str, Any], explorer_param_name: str, value: float):
        ...
        if explorer_param_name == 'water_fraction':
            ...
            converted_value = max(converted_value, 1e-6)  # Prevent zero/negative areas
        elif 'mass' in explorer_param_name.lower():
            converted_value = max(converted_value, 0.0)  # Prevent negative mass
        elif 'pressure' in explorer_param_name.lower():
            converted_value = max(converted_value, ATMOSPHERIC_PRESSURE)  # Minimum atmospheric pressure
    ...
    def extract_base_parameters(self, rocket) -> Dict[str, float]:
        ...
        for explorer_param_name, mapping_info in self.parameter_mappings.items():
            ...
            if sim_param_key in sim_params:
                ...
                if 'diameter' in explorer_param_name and 'area' in sim_param_key.lower():
    ...
    def create_parameter_configs(self, base_params: Dict[str, float], 
                               parameter_names: List[str],
                               custom_ranges: Dict[str, Dict] = None) -> Dict[str, ParameterConfig]:
        """Create parameter configurations for exploration."""
        configs = {}
        
        for param_name in parameter_names:
            if param_name not in base_params:
                raise ValueError(f"Parameter '{param_name}' not found in base parameters. Available: {list(base_params.keys())}")
            
            if param_name not in self.parameter_mappings:
                raise ValueError(f"Parameter '{param_name}' not defined in parameter mappings. Available: {list(self.parameter_mappings.keys())}")
            ...
            configs[param_name] = ParameterConfig(
                ...
                unit=unit,
                **{k: v for k, v in range_config.items() if k != 'convert_func'}
            )
        
        return configs
    
    def simulate_single_point(self, base_rocket, param_values: Dict[str, float], 
                            sim_settings: Dict[str, Any] = None) -> Optional[Any]:
        ...
        try:
            ...
            print(f"Debug - Original sim_params keys: {list(sim_params.keys())}")
            
            # Update parameters using the generic updater
            for param_name, value in param_values.items():
                self._update_parameter(sim_params, param_name, value)
            
            print(f"Debug - Updated sim_params keys: {list(sim_params.keys())}")
            ...
            warnings.warn(f"Simulation failed for parameters {param_values}: {e}")
            print(f"Debug - Exception details: {type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
            return None
    
    def explore_single_parameter(self, base_rocket, param_config: ParameterConfig,
                               target: str = 'apogee',
                               sim_settings: Dict[str, Any] = None) -> ExplorationResult:
        """Explore a single parameter."""
        print(f"Exploring parameter: {param_config.name}")
        
        target_attr, target_unit, target_display = self.target_extractors[target]
        
        # Get base target value
        base_flight_data = self.simulate_single_point(base_rocket, {}, sim_settings)
    ...
    def explore_multiple_parameters(self, base_rocket, param_configs: Dict[str, ParameterConfig],
                                  target: str = 'apogee',
                                  sim_settings: Dict[str, Any] = None,
                                  use_parallel: bool = True) -> List[ExplorationResult]:
        """Explore multiple parameters with pairwise combinations."""
        
        print(f"Exploring {len(param_configs)} parameters: {list(param_configs.keys())}")
        ...
        for param1_name, param2_name in combinations(param_names, 2):
            ...
            for i in range(P1.shape[0]):
                for j in range(P1.shape[1]):
                    ...
                    flight_data = self.simulate_single_point(base_rocket, param_values, sim_settings)
                    ...
                    if completed_sims % 10 == 0:
                        print(f"    Progress: {completed_sims}/{total_sims} ({100*completed_sims/total_sims:.1f}%)")
            
            # Calculate sensitivities for both parameters
            sensitivity1 = self._calculate_sensitivity(base_rocket, param1_config, target, base_target_value, sim_settings)
            sensitivity2 = self._calculate_sensitivity(base_rocket, param2_config, target, base_target_value, sim_settings)
            
            results.append(ExplorationResult(
                parameter_names=[param1_name, param2_name],
                parameter_values={param1_name: p1_values, param2_name: p2_values},
                target_values=target_grid,
                target_name=target_display,
                target_unit=target_unit,
                base_target_value=base_target_value,
                sensitivity_analysis={param1_name: sensitivity1, param2_name: sensitivity2}
            ))
        
        return results
    
    def _calculate_sensitivity(self, base_rocket, param_config: ParameterConfig, 
                             target: str, base_target_value: float,
                             sim_settings: Dict[str, Any] = None) -> float:
    ...
    def plot_results(self, results: List[ExplorationResult], save_plots: bool = False):
    ...
    def _plot_single_parameter(self, result: ExplorationResult, save_plots: bool, plot_idx: int):
        ...
        plt.axhline(y=result.base_target_value, color='red', linestyle='--', alpha=0.7, label='Base Value')
        
        plt.xlabel(f'{param_name.replace("_", " ").title()}')
        plt.ylabel(f'{result.target_name} ({result.target_unit})')
        plt.title(f'{result.target_name} vs {param_name.replace("_", " ").title()}')
        plt.grid(True, alpha=0.3)
        plt.legend()
        
        # Add sensitivity annotation
        sensitivity = result.sensitivity_analysis[param_name]
        plt.text(0.05, 0.95, f'Sensitivity: {sensitivity:.2e} {result.target_unit}/unit', 
        ...
        if save_plots:
            plt.savefig(f'parameter_exploration_{plot_idx}_{param_name}.png', dpi=300, bbox_inches='tight')
        
        plt.show()
    
    def _plot_two_parameters(self, result: ExplorationResult, save_plots: bool, plot_idx: int):
        ...
        contour = ax1.contour(P1, P2, result.target_values, levels=15, colors='black', alpha=0.5, linewidths=0.5)
        contourf = ax1.contourf(P1, P2, result.target_values, levels=20, cmap='viridis', alpha=0.8)
        ...
        surface = ax2.plot_surface(P1, P2, result.target_values, cmap='viridis', alpha=0.8)
        ...
        if save_plots:
            plt.savefig(f'parameter_exploration_{plot_idx}_{param1_name}_{param2_name}.png', 
    ...
    def get_available_parameters(self) -> Dict[str, str]:
        ...
        for param_name, mapping in self.parameter_mappings.items():
            sim_param_key, display_name, unit = mapping[:3]
            available[param_name] = f"{display_name} ({unit}) -> {sim_param_key}"
        return available
    
    def add_parameter_mapping(self, explorer_name: str, sim_param_key: str, 
                            display_name: str, unit: str, 
                            range_config: Dict = None, convert_func: Callable = None):
        ...
        avg_sensitivities = {name: np.mean(values) for name, values in all_sensitivities.items()}
        ...
        # Relative importance
        max_sensitivity = max(avg_sensitivities.values()) if avg_sensitivities else 1
...
def main():
    ...
    param_configs = explorer.create_parameter_configs(
        ...
        custom_ranges={
            'initial_pressure': {'min_factor': 0.4, 'max_factor': 2.5, 'num_points': 8},
            'water_fraction': {'min_factor': 0.5, 'max_factor': 1.8, 'num_points': 8},
            'nozzle_diameter': {'min_factor': 0.6, 'max_factor': 2.0, 'num_points': 8}
        }
    )
