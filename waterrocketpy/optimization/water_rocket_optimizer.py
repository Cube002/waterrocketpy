...
class WaterRocketOptimizer:
    ...
    def objective_function(self, params: np.ndarray, target: str = 'max_altitude') -> float:
        """
        ...
        if cache_key in self._simulation_cache:
            ...
            try:
                ...
                # Run simulation
                flight_data = self.simulator.simulate(sim_params, self.simulation_settings)
                ...
                warnings.warn(f"Simulation failed with parameters {params}: {e}")
                return 1e6
        ...
        if self.best_result is None or result > self.best_result['result']:
            ...
            print(f"New best {target}: {result:.4f} at evaluation {self.n_evaluations}")
            print(f"  Params: L_body={params[0]:.3f}, d_body={params[1]:.3f}, "
    ...
    def optimize(self, 
        ...
        optimization_result = {
            ...
            'target': target,
            'scipy_result': result,
            'best_flight_data': self.best_result['flight_data'] if self.best_result else None
        }
...
def optimize_for_altitude(bounds: Optional[List[Tuple[float, float]]] = None,
                         method: str = 'differential_evolution',
                         **kwargs) -> Dict:
    ...
    return optimizer.optimize(bounds, target='max_altitude', method=method, **kwargs)


def optimize_for_velocity(bounds: Optional[List[Tuple[float, float]]] = None,
                         method: str = 'differential_evolution', 
                         **kwargs) -> Dict:
    ...
    if bounds is None:
        bounds = optimizer.get_default_bounds()
    return optimizer.optimize(bounds, target='max_velocity', method=method, **kwargs)


def optimize_for_flight_time(bounds: Optional[List[Tuple[float, float]]] = None,
                            method: str = 'differential_evolution',
                            **kwargs) -> Dict:
    ...
    if bounds is None:
        bounds = optimizer.get_default_bounds()
    return optimizer.optimize(bounds, target='flight_time', method=method, **kwargs)
...
if __name__ == "__main__":
    ...
    # Example 3: Optimize using minimize method instead of differential_evolution
    print("\nExample 3: Optimizing flight time using minimize method")
    result_flight_time = optimize_for_flight_time(
        method='minimize',
        x0=[0.25, 0.088, 8.0, 0.01, 0.3]  # Initial guess
    )
