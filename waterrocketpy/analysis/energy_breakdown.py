...
def tenergy_breakdown(flight_data, rocket_params: Dict[str, Any]) -> EnergyComponents:
    """
    ...
    for i in range(1, len(time)):
        ...
        if dm_water_dt > 0:  # Water is being expelled
            ...
            if flight_data.water_exhaust_speed[i] is not None:
                ...
                if dm_water_dt > 0:
                    v_exhaust = thrust[i] / dm_water_dt if dm_water_dt > 1e-10 else 0.0
                else:
                    v_exhaust = 0.0
            ...
            water_out_kinetic_energy[i] = water_out_kinetic_energy[i-1] + dE_kin_water
            water_out_potential_energy[i] = water_out_potential_energy[i-1] + dE_pot_water
        else:
    ...
    for i in range(1, len(time)):
        ...
        if dm_air_dt > 0:  # Air is being expelled
            ...
            if flight_data.air_exhaust_speed[i] is not None:
                ...
                # Fallback: estimate from thrust and mass flow rate
                if dm_air_dt > 0:
                    v_exhaust_air = thrust[i] / dm_air_dt if dm_air_dt > 1e-10 else 0.0
                else:
                    v_exhaust_air = 0.0
            ...
            air_out_kinetic_energy[i] = air_out_kinetic_energy[i-1] + dE_kin_air
            air_out_potential_energy[i] = air_out_potential_energy[i-1] + dE_pot_air
        else:
    ...
    initial_air_mass = (rocket_params['P0'] * initial_air_volume) / (287.0 * INITIAL_TEMPERATURE)  # Using ideal gas law
    total_initial_energy = initial_air_mass * AIR_SPECIFIC_HEAT_CV * INITIAL_TEMPERATURE
    
    # 10. Energy Conservation Check
    # Total energy at time t should equal:
    # Internal + Kinetic + Potential + Expelled + Losses
    total_final_energy = (air_internal_energy + 
                         rocket_kinetic_energy + rocket_potential_energy +
                         water_out_kinetic_energy + water_out_potential_energy +
                         air_out_kinetic_energy + air_out_potential_energy +
                         drag_energy_loss)
    
    energy_conservation_error = abs(total_final_energy - total_initial_energy) / total_initial_energy * 100
    ...
    total_expelled_energy = (water_out_kinetic_energy[-1] + water_out_potential_energy[-1] +
                           air_out_kinetic_energy[-1] + air_out_potential_energy[-1])
